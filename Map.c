...
static int validV(Map, Location);
static VList newNode(Location, Transport, VList);
...
void addLink(Map g, Location start, Location end, Transport type) {
    // Task 2
    assert(validV(g, start) && validV(g, end));
    assert(type <= NUM_TRANSPORT && type > NONE);
    assert(start != end);

    g->connections[start] = newNode(end, type, g->connections[start]);
    g->connections[end] = newNode(start, type, g->connections[end]);

    g->nE++;
}

//written to aid addLink()
//check that the given location is not null, and is less than #vertices
static int validV(Map g, Location v){
    return (g != NULL && v >= 0 && v < g->nV);
}

//addLink() helper function.
//create new node for the linked list
static VList newNode(Location v, Transport type, VList next) {
